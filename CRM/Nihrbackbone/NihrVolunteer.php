...
/*
  ...
  /**
   * Method to remove HLQ data
   * @param int $contactId
   */
  public static function removeHlqData(int $contactId) {
    // *** delete HLQ data, if flag "request_to_destroy_data" is set
    try {
      \Civi\Api4\Contact::update()
        ->addWhere('id', '=', $contactId)
        ->addValue('gender_id', '')
    ...
    try {
      \Civi\Api4\CustomValue::delete('nihr_volunteer_life_quality')
        ->addWhere('entity_id', '=', $contactId)
        ->execute();
    } catch (API_Exception $ex) {
    }
    ...
    try {
      \Civi\Api4\CustomValue::delete('nihr_volunteer_general_observations')
        ->addWhere('entity_id', '=', $contactId)
        ->execute();
    } catch (API_Exception $ex) {
    }
